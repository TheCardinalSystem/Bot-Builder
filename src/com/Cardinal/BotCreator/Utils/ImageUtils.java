package com.Cardinal.BotCreator.Utils;
...
public class ImageUtils {
	...
	public static Image resizeAspectRatio(Image image, Dimension desiredResize) {
		...
		double h = (double) new ImageIcon(image).getIconHeight();
		...
		if (h > nh || w > nw) {
			if (h > w) {
				...
				double tallCake = (double) h * (double) percentH;
				double fatCake = (double) w * (double) percentH;
				System.out.println(tallCake + ":" + fatCake);

				String newTallCake = String.valueOf(tallCake / 100).substring(0,
						String.valueOf(tallCake / 100).indexOf("."));
				String newFatCake = String.valueOf(fatCake / 100).substring(0,
						String.valueOf(fatCake / 100).indexOf("."));

				System.err.println(newTallCake + ":" + newFatCake);

				return resizeImage(image, Integer.parseInt(newFatCake), Integer.parseInt(newTallCake));
			} else if (w > h) {
				double percentW = (nw / w) * (double) 100;
				System.out.println(percentW);
				double tallCake = (double) h * (double) percentW;
				double fatCake = (double) w * (double) percentW;
				System.out.println(tallCake + ":" + fatCake);
